...
public:
    ...
    bool wound_rebirth(ts_t ts, txn_man *curr_txn, TsType type) {
        ...
        // find writing txns whose timestamp priority lower than me
        uint64_t starttimerebirth = get_sys_clock();
        std::vector<Version *> lower_than_me;  // like: [120, 119, 112, 100]
        ...
        if (lower_than_me.empty()) {
#if PF_CS
            INC_STATS(curr_txn->get_thd_id(), time_rebirth,  get_sys_clock() - starttimerebirth);
#endif
            return false;
        }
        ...
        if (size_dep > 0 && size_sort > 1){
            ...
            auto max_ts = ts;
            for (int i = 0; i < lower_than_me.size(); ++i) {
